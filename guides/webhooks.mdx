...
- **Por que existe:** consultar `GET /api/onboarding/report/{tokenOnboarding}`
...
- **Pra que serve pra você:** é a forma recomendada de acompanhar onboarding
...
- Responda rápido (idealmente abaixo de alguns segundos) e processe o
  conteúdo depois, de forma assíncrona. Um endpoint lento pode ser
  interpretado como falha.
- Sempre valide o header `id-cerberus-token` antes de confiar no payload.
- Trate eventos repetidos. Reprocessar o mesmo `tokenOnboarding` com o mesmo
  status não deve causar efeito colateral duplicado.
- Não trate `REFUSED` como erro técnico — é um resultado de negócio, igual ao
  consumo avulso via polling. Veja
  [Status e erros](/guides/status-e-erros).
- Mantenha um fallback de polling. Como não existe reenvio automático para
