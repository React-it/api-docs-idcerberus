...
- **O que é:** uma consulta ou verificação específica, como "situação do CPF na
...
- **Por que existem mais de 300:** cada tipo de dado ou verificação vira um
  service diferente, porque cada empresa usa uma combinação diferente deles.
  Você nunca precisa de todos.
- **Pra que serve pra você:** é a unidade que você efetivamente chama. Toda
...
- **O que é:** duas formas de usar um service. Avulsa é chamar um service
...
- **Por que as duas existem:** nem toda necessidade é um cadastro completo.
...
- **Pra que serve pra você:** define o modelo de integração que você vai
...
- **O que é:** a configuração específica da sua empresa — quais services estão
  liberados, em qual ordem entram no onboarding e quais ficam disponíveis para
  consulta avulsa.
- **Por que existe:** cada cliente da idCerberus usa um conjunto diferente de
  services. O produto é o que amarra "API idCerberus" a "o que a sua empresa
  efetivamente contratou".
- **Pra que serve pra você:** você nunca escolhe entre todos os 300+ services —
...
- [Backoffice](/guides/backoffice): interface visual da idCerberus, para
  olhar resultados e testar consultas sem escrever código.
- [Webhooks](/guides/webhooks): avisa sua aplicação quando o status mudar —
